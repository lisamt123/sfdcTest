({
    ...
    doSearch : function(cmp) {
        ...
        action.setAbortable();
        //where condition
        var where1 = cmp.get('v.where');
        console.log('where1>>>>>>>>>'+where1);
        if(where1.search("''") != -1){
            where1 = 'none' ;
        }
        if(where1.search("null") != -1){
            where1 = 'none' ;
        }
        // Set the parameters
        action.setParams({ "searchString" : searchString, "sObjectAPIName" : sObjectAPIName, "where1" : where1 });
    ...
    renderLookupComponents : function(cmp, lookupListItems, matches)
    {
        ...
        console.log('-listIconClass-'+listIconClass);
        console.log('-listIconSVGPath-'+listIconSVGPath);
    ...
    handleSelection : function(cmp, event) {
        ...
        var objectLabel = selectedItem.dataset.label;
        
        // Log the Object Id and Label to the console
        document.getElementById('valueID').value = objectId;
